...
def handler (event, context):
    ...
    for rolename in iam_roles:
        ...
        if policy_arn in role_policies:
            ...
            non_compliant_roles.append (rolename)

    report = {
        'NonCompliantUsers': non_compliant_users,
        'NonCompliantRoles': non_compliant_roles
    }

    sns.publish (TopicArn = topic_arn, Subject = 'IAM Compliance report', Message = json.dumps (report))
